...
def e6_key_from_json(user_addr: str, server: Server, sub_repo) -> Optional[hallo.modules.subscriptions.common_e6_key.E6Key]:
    ...
    e6_key = e6_keys.get_key_by_user(user)
    return e6_key


def e6_key_from_input(user: User, sub_repo) -> Optional[hallo.modules.subscriptions.common_e6_key.E6Key]:
    e6_keys = sub_repo.get_common_config_by_type(hallo.modules.subscriptions.common_e6_key.E6KeysCommon)
    e6_key = e6_keys.get_key_by_user(user)
    return e6_key
...
class E621Source(hallo.modules.subscriptions.stream_source.StreamSource[Post]):
    ...
    def from_input(cls, argument: str, user: User, sub_repo) -> 'E621Source':
        ...
        e6_key = e6_key_from_input(user, sub_repo)
        return E621Source(
            argument,
            e6_client,
            e6_key
    ...
    def current_state(self) -> List[Post]:
        return self.e6_client.posts(self.search, self.e6_key)
    ...
    def from_json(cls, json_data: Dict, destination: Destination, sub_repo) -> 'E621Source':
        ...
        e6_key = e6_key_from_json(user_addr, destination.server, sub_repo)
        e6_client = sub_repo.get_common_config_by_type(hallo.modules.subscriptions.common_e6_key.E6ClientCommon)
        return E621Source(
            json_data["search"],
            e6_client,
