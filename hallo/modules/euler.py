...
class Euler(Function):
    ...
    def euler37(self):
        ...
        while num_found < 11 and number < 10 ** 7:
            ...
            if truncatable:
                logger.debug("found one. {}".format(str_number))
                num_found += 1
                total += number
            number += 1
        return total
        ...
        for x in range(10 ** 5):
            ...
            if len(constr) == 9 and int(constr) > max_constr:
                if self.check_list_in_list(
                    ...
                    logger.debug("new max: {}".format(constr))
        ...
        for x in range(2, 8 * 10 ** 6):
            ...
            if x % 2 != 0 and x % 3 != 0 and x % 5 != 0:
                if self.check_list_in_list(
                    ...
                    if self.check_prime(x):
                        logger.debug("this is one. {}".format(x))
        ...
        for pandigital in pandigitals:
            if (
                ...
                if (
                    ...
                    logger.debug("found one: ".format(pandigital))
    ...
    def euler54(self):
        ...
        for poker_game in list_poker_games:
            deck = hallo.modules.games.cards.Deck()
            hand_one = hallo.modules.games.cards.Hand.from_two_letter_code_list(deck, poker_game.split()[:5])
            hand_two = hallo.modules.games.cards.Hand.from_two_letter_code_list(deck, poker_game.split()[5:])
    ...
    def euler59(self):
        ...
        logger.debug("Total: {}".format(sum_values))
        return sum_values
