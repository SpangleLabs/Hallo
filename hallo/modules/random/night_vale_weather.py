...
class NightValeWeather(Function):
    ...
    async def run(self, event):
        ...
        try:
            playlist_data = await self.get_youtube_playlist("PL1-VZZ6QMhCdx8eC4R3VlCmSn1Kq2QWGP")
    ...
    async def get_youtube_playlist(self, playlist_id, page_token=None):
        ...
        if "nextPageToken" in api_dict:
            list_videos.extend(
                await self.get_youtube_playlist(playlist_id, api_dict["nextPageToken"])
            )
        # Return list
        return list_videos
