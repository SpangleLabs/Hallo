...
class Voice(Function):
    ...
    async def run(self, event):
        ...
        if len(line_split) == 0:
            ...
            # Give user voice
            return event.create_response(await self.give_voice(event.channel, event.user))
        ...
        if len(line_split) == 1:
            ...
            if event.channel is None:
                ...
                return event.create_response(await self.give_voice(channel, event.user))
            ...
            if test_channel is not None and test_channel.in_channel:
                return event.create_response(await self.give_voice(test_channel, event.user))
            # Argument must be a user?
            target_user = server_obj.get_user_by_name(event.command_args)
            if target_user is None:
                return event.create_response(f"Error, {event.command_args} is not known on {server_obj.name}.")
            return event.create_response(await self.give_voice(event.channel, target_user))
        ...
        if target_channel is not None and target_channel.in_channel:
            ...
            return event.create_response(await self.give_voice(target_channel, target_user))
    ...
    async def give_voice(self, channel: Channel, user: User) -> str:
        """
        ...
        if not hallo.modules.channel_control.channel_control.hallo_has_op(channel):
            return f"Error, I don't have power to give voice in {channel.name}."
        ...
        mode_evt = EventMode(channel.server, channel, None, f"+v {user.address}", inbound=False)
...
class DeVoice(Function):
    ...
    async def run(self, event):
        ...
        if len(line_split) == 0:
            ...
            # Give user voice
            return event.create_response(await self.take_voice(event.channel, event.user))
        # If 1 argument, see if it's a channel or a user.
        if len(line_split) == 1:
            # If message was sent in private message, it's referring to a channel
            if event.channel is None:
                ...
                if channel is None:
                    return event.create_response(f"Error, {event.command_args} is not known on {server_obj.name}.")
                return event.create_response(await self.take_voice(channel, event.user))
            # See if it's a channel that hallo is in
            test_channel = server_obj.get_channel_by_name(event.command_args)
            if test_channel is not None and test_channel.in_channel:
                return event.create_response(await self.take_voice(test_channel, event.user))
            # Argument must be a user?
            target_user = server_obj.get_user_by_name(event.command_args)
            if target_user is None:
                return event.create_response(f"Error, {event.command_args} is not known on {server_obj.name}.")
            return event.create_response(await self.take_voice(event.channel, target_user))
        # If 2 arguments, try with first argument as channel
        target_channel = server_obj.get_channel_by_name(line_split[0])
        if target_channel is not None and target_channel.in_channel:
            target_user = server_obj.get_user_by_name(line_split[1])
            if target_user is None:
                return event.create_response(f"Error, {line_split[1]} is not known on {server_obj.name}.")
            return event.create_response(await self.take_voice(target_channel, target_user))
        # 2 args, try with second argument as channel
        target_user = server_obj.get_user_by_name(line_split[0])
        if target_user is None:
            return event.create_response(f"Error, {line_split[0]} is not known on {server_obj.name}.")
    ...
    async def take_voice(self, channel: Channel, user: User) -> str:
        """
        ...
        # Check if hallo has op in channel
        if not hallo.modules.channel_control.channel_control.hallo_has_op(channel):
            return f"Error, I don't have power to take voice in {channel.name}."
        # Check that user does not have op in channel
        user_membership = channel.get_membership_by_user(user)
        if not user_membership.is_voice:
            return "Error, this user doesn't have voice."
        mode_evt = EventMode(channel.server, channel, None, f"-v {user.address}")
        await channel.server.send(mode_evt)
