    ...
    async def run(self, event):
        ...
        if len(line_split) == 1:
            ...
            if event.channel is None:
                ...
                if channel is None:
                    return event.create_response(
                        f"Error, {event.command_args} is not known on {server_obj.name}."
                    )
                return event.create_response(await self.send_kick(channel, event.user))
            ...
            if test_channel is not None and test_channel.in_channel:
                return event.create_response(await self.send_kick(test_channel, event.user))
            ...
            return event.create_response(await self.send_kick(event.channel, target_user))
        if len(line_split) == 2:
                ...
                if target_channel is not None:
                    if target_channel.in_channel:
                        ...
                        if (
                            ...
                            return event.create_response(
                                await self.send_kick(target_channel, target_user)
                            )
                        return event.create_response(
                            await self.send_kick(target_channel, event.user, line_split[1])
                        )
            ...
            return event.create_response(
                await self.send_kick(event.channel, target_user, line_split[1])
            )
        ...
        if event.channel is None:
            ...
            if target_user is None:
                return event.create_response(
                    f"Error, {line_split[0]} is not known on {server_obj.name}."
                )
            target_channel = server_obj.get_channel_by_name(line_split[1])
            if target_channel is None:
                return event.create_response(
            ...
            return event.create_response(
                await self.send_kick(target_channel, target_user, " ".join(line_split[2:]))
            )
        ...
        if target_channel is not None and target_channel.in_channel:
            ...
            return event.create_response(
                await self.send_kick(target_channel, event.user, " ".join(line_split[1:]))
            )
        # 2 args, try with second argument as channel
        target_user = server_obj.get_user_by_name(line_split[0])
        if target_user is None:
        ...
        return event.create_response(
            await self.send_kick(event.channel, target_user, " ".join(line_split[1:]))
        )

    async def send_kick(self, channel: Channel, user: User, message: str = "") -> str:
        """
        ...
        if not hallo.modules.channel_control.channel_control.hallo_has_op(channel):
            return f"Error, I don't have power to kick users from {channel.name}."
        # Send invite
        kick_evt = EventKick(
            channel.server, channel, None, user, message, inbound=False
        )
        await channel.server.send(kick_evt)
        return f"Kicked {user.name} from {channel.name}."
