...
class Hailstone(Function):
    ...
    def run(self, event):
        ...
        if number > Hailstone.LIMIT:
            return event.create_response(
                "Error, this is above the limit for this function."
            )
        sequence = self.collatz_sequence([number])
        output_string = "Hailstone (Collatz) sequence for {}: " "{} ({} steps.)".format(
            number, "->".join(str(x) for x in sequence), len(sequence)
        )
...
class PrimeFactors(Function):
    ...
    def run(self, event):
        ...
        if line_clean.isdigit():
            ...
            return event.create_response(
                "Error, this is not a valid number or calculation."
            )
        prime_factors = self.find_prime_factors(number)
        return event.create_response(
            "The prime factors of {} are: {}.".format(
                number, "x".join(str(x) for x in prime_factors)
            )
        )
...
class Average(Function):
    ...
    def run(self, event):
        ...
        return event.create_response(
            "The average of {} is: {}.".format(
                ", ".join(number_list), number_sum / float(len(number_list))
            )
        )
...
class HighestCommonFactor(Function):
    ...
    def run(self, event):
        ...
        prime_factors_obj = function_dispatcher.get_function_object(
            prime_factors_class
        )  # type: PrimeFactors
        ...
        common_factors = simp_frac_obj.list_intersection(
            number_one_factors, number_two_factors
        )
        hcf = simp_frac_obj.list_product(common_factors)
        return event.create_response(
            "The highest common factor of {} and {} is {}.".format(
                number_one, number_two, hcf
            )
        )
...
class SimplifyFraction(Function):
    ...
    def run(self, event):
        ...
        if event.command_args.count("/") != 1:
            return event.create_response(
                "Error, Please give input in the form: <numerator>/<denominator>"
            )
        ...
        numerator_factors_new = self.list_minus(
            numerator_factors,
            self.list_intersection(denominator_factors, numerator_factors),
        )
        ...
        denominator_out = "{}{}/{}".format(
            "-" * negative, numerator_new, denominator_new
        )
...
class Calculate(Function):
    ...
    def run(self, event):
        ...
        if calc.count("=") >= 1:
            ...
            if number_answers and number_answers.count(number_answers[0]) != len(
                number_answers
            ):
    ...
    def passive_run(self, event, hallo_obj):
        ...
        if Commons.check_numbers(event.text.replace(".", "")):
            return None
        if not any(
            [char in event.text for char in [str(x) for x in range(10)] + ["e", "pi"]]
        ):
            return None
    ...
    def process_calculation(self, calc):
        ...
        while calc.count("^") != 0:
            ...
            calc = calc.replace(
                "{}^{}".format(pre_calc, post_calc),
                str(float(pre_calc) ** float(post_calc)),
                1,
            )
