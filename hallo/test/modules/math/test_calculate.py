...
def test_calc_simple(hallo_getter):
    ...
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc 2+2")
    )
...
def test_calc_multiply(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc 21*56")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_calc_divide(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc 1/5")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_calc_subtract(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc 13-17")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_calc_div_zero(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc 1/0")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_cos(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc cos(0)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 1 == float(data[0].text), "cos(0) != 1"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc cos(pi/2)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 0 == float(data[0].text), "cos(pi/2) != 0"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc cos(pi)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_sin(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc sin(0)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 0 == float(data[0].text), "sin(0) != 0"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc sin(pi/2)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 1 == float(data[0].text), "sin(pi/2) != 0"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc sin(pi)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_order_of_operations(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc 6+7*8")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 62 == float(data[0].text), "6+7*8 != 62"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc 16/8-2")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 0 == float(data[0].text), "16/8-2 != 0"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc 9-5/(8-3)*2+6")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 13 == float(data[0].text), "9-5/(8-3)*2+6 != 13"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc 150/(6+3*8)-5")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_brackets(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc (25-11)*3")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 42 == float(data[0].text), "(25-11)*3 != 42"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc 4+(-1(-2-1))^2")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 13 == float(data[0].text), "4+(-1(-2-1))^2 != 13"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc 2(3+4)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 14 == float(data[0].text), "2(3+4) != 14"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc (3+4)3")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 21 == float(data[0].text), "(3+4)3 != 21"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc (((17*3)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert "error" in data[0].text.lower(), "(((17*3) should fail"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc (21/3))+2))*5")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert "error" in data[0].text.lower(), "(21/3))+2))*5 should fail"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc ((15*(3))())")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert "error" in data[0].text.lower(), "((15*(3))()) should fail"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc (3)-(7)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert -4 == float(data[0].text), "(3)-(7) != -4"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc e(3+4)pi")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 59.778 == float(data[0].text[:6]), "e(3+4)pi != 59.778"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc cos(acos(0.7))")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_pi(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc pi")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_e(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc e")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_tan(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc tan(0)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 0 == float(data[0].text), "tan(0) != 0"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc tan(pi)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 0 == float(data[0].text), "tan(pi) != 0"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc tan(pi/2)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_acos(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc acos(1)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 0 == float(data[0].text), "acos(1) != 0"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc acos(0)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    ...
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc acos(2)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_asin(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc asin(0)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 0 == float(data[0].text), "asin(0) != 0"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc asin(1)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    ...
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc asin(2)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_atan(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc atan(0)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 0 == float(data[0].text), "atan(0) != 0"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc atan(1)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    ...
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc atan(1000000)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_sqrt(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc sqrt(4)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 2 == float(data[0].text), "sqrt(4) != 2"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc sqrt(2)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert "1.414" == data[0].text[:5], "sqrt(2) != 1.414"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc sqrt(1)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 1 == float(data[0].text), "sqrt(1) != 1"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc sqrt(2.25)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_power(hallo_getter):
    ...
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc 2^-1")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 0.5 == float(data[0].text), "2^-1 != 1/2"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc 2^0.5")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 1.414 == float(data[0].text[:5]), "2^0.5 != 1.414"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc 2^0")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_hyperbolics(hallo_getter):
    ...
    # Cosh
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc cosh(0)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 1 == float(data[0].text), "cosh(0) != 1"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc cosh(1)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    ...
    # Sinh
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc sinh(0)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 0 == float(data[0].text), "sinh(0) != 0"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc sinh(1)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    ...
    # Tanh
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc tanh(0)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 0 == float(data[0].text), "tanh(0) != 0"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc tanh(1)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    ...
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc tanh(1000000)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 1 == float(data[0].text), "tanh(1000000) != 1"
    # Acosh
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc acosh(0)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert "error" in data[0].text.lower(), "acosh(0) should fail"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc acosh(1)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 0 == float(data[0].text), "acosh(1) != 0"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc acosh(4)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 2.063 == float(data[0].text[:5]), "acosh(4) != 2.063"
    # Asinh
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc asinh(0)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 0 == float(data[0].text), "asinh(0) != 0"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc asinh(1)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    ...
    # Atanh
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc atanh(0)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 0 == float(data[0].text), "atanh(0) != 0"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc atanh(0.5)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    ...
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc atanh(1)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert "error" in data[0].text.lower(), "atanh(1) should fail"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc atanh(2)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_gamma(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc gamma(1)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 1 == float(data[0].text), "gamma(1) != 1"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc gamma(5)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert 24 == float(data[0].text), "gamma(5) != 24"
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc gamma(0)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_passive(hallo_getter):
    ...
    hallo.function_dispatcher.dispatch_passive(
        EventMessage(test_server, test_channel, test_user, "25")
    )
    data = test_server.get_send_data(0)
    assert len(data) == 0, "No response should have happened."
    hallo.function_dispatcher.dispatch_passive(
        EventMessage(test_server, test_channel, test_user, "23.47")
    )
    data = test_server.get_send_data(0)
    assert len(data) == 0, "No response should have happened."
    hallo.function_dispatcher.dispatch_passive(
        EventMessage(test_server, test_channel, test_user, "2+2")
    )
    data = test_server.get_send_data(1, test_channel, EventMessage)
    assert data[0].text == "4", "2+2 = 4, hallo should have responded"
    hallo.function_dispatcher.dispatch_passive(
        EventMessage(test_server, test_channel, test_user, "pie")
    )
    data = test_server.get_send_data(1, test_channel, EventMessage)
    ...
    hallo.function_dispatcher.dispatch_passive(
        EventMessage(test_server, test_channel, test_user, "acos(2)")
    )
    data = test_server.get_send_data(0)
    assert len(data) == 0, "No response should have been received"
    hallo.function_dispatcher.dispatch_passive(
        EventMessage(test_server, test_channel, test_user, " 97")
    )
    data = test_server.get_send_data(0)
    assert len(data) == 0, "No response should have been received"
    hallo.function_dispatcher.dispatch_passive(
        EventMessage(test_server, test_channel, test_user, "9 7")
    )
    data = test_server.get_send_data(0)
    assert len(data) == 0, "No response should have been received"
...
def test_passive_ip_error(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch_passive(
        EventMessage(test_server, test_channel, test_user, "127.0.0.1")
    )
    data = test_server.get_send_data(0)
    assert len(data) == 0, "No response should have happened."
...
def test_ee(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc ee")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    ...
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc pipi")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert data[0].text != "0", "Improper processing of constants."
    assert (
...
def test_equals(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc 2+2=4")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    ...
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc 2+2=5")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    ...
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc pi=acos(-1)")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    ...
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc circle constant=pi")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    ...
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc hello=goodbye")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
    ...
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, None, test_user, "calc x=2+2=y=5")
    )
    data = test_server.get_send_data(1, test_user, EventMessage)
