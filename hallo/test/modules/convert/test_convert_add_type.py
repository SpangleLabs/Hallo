...
class ConvertAddTypeTest(ConvertFunctionTestBase, unittest.TestCase):
    ...
    def test_no_base_unit(self):
        ...
        data = self.server.get_send_data(1, self.test_user, EventMessage)
        assert (
            "must define a base unit" in data[0].text
        ), "Should warn a base unit is needed"
        assert (
            "unit=<unit name>" in data[0].text
        ), "Should specify syntax to specify base unit"
        assert (
            len(self.test_repo.type_list) == num_types
        ), "Shouldn't have added a new type"
    ...
    def test_create_new_type(self):
        ...
        data = self.server.get_send_data(1, self.test_user, EventMessage)
        assert (
            'created new type "new_type"' in data[0].text.lower()
        ), "Should specify new type name"
        assert (
            'with base unit "unit2".' in data[0].text
        ), "Should specify base unit, and not decimals"
        ...
        assert (
            new_type.base_unit.name_list[0] == "unit2"
