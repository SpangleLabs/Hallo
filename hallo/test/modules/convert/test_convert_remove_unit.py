...
class ConvertRemoveUnitTest(ConvertFunctionTestBase, unittest.TestCase):
    def test_unrecognised_type(self):
        ...
        assert (
            "type is not recognised" in data[0].text
        ), "Error should say type does not exist"
        assert (
    ...
    def test_specified_type_unrecognised_unit(self):
        ...
        data = self.server.get_send_data(1, self.test_user, EventMessage)
        assert (
            "unit name is not recognised for that unit type" in data[0].text
    ...
    def test_specified_type_but_its_base_unit(self):
        ...
        data = self.server.get_send_data(1, self.test_user, EventMessage)
        assert (
            "cannot remove the base unit for a unit type" in data[0].text
    ...
    def test_specified_type_removes_unit(self):
        ...
        data = self.server.get_send_data(1, self.test_user, EventMessage)
        assert (
            'removed unit "unit1b"' in data[0].text.lower()
        ), "Didn't remove unit correctly"
        assert (
    ...
    def test_no_unit_no_type(self):
        ...
        data = self.server.get_send_data(1, self.test_user, EventMessage)
        assert (
            "no unit by that name" in data[0].text.lower()
        ), "Error should say unit not found"
    ...
    def test_multiple_matching_units(self):
        ...
        data = self.server.get_send_data(1, self.test_user, EventMessage)
        assert (
            "more than one unit matching this name" in data[0].text.lower()
        ), "Error should say multiple units match"
        assert (
            "unit1b (type=test_type1)" in data[0].text.lower()
        ), "Should have suggested unit1b"
        assert (
            "unit2b (type=test_type2)" in data[0].text.lower()
        ), "Should have suggested unit2b"
        assert (
            len(self.test_type1.unit_list) == type1_units
        ), "Shouldn't have removed the unit from type1"
        assert (
