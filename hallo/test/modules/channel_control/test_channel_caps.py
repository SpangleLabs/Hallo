...
def test_caps_toggle(hallo_getter):
    ...
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, test_chan, test_user, "channel caps")
    )
...
def test_caps_on(hallo_getter):
    ...
    test_chan.use_caps_lock = False
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, test_chan, test_user, "channel caps on")
    )
    data = test_server.get_send_data(1, test_chan, EventMessage)
    assert "error" not in data[0].text.lower()
...
def test_caps_off(hallo_getter):
    ...
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, test_chan, test_user, "channel caps off")
    )
    data = test_server.get_send_data(1, test_chan, EventMessage)
    assert "error" not in data[0].text.lower()
...
def test_caps_channel_toggle(hallo_getter):
    ...
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, test_chan, test_user, "channel caps other_channel")
    )
    data = test_server.get_send_data(1, test_chan, EventMessage)
    assert "error" not in data[0].text.lower()
    assert "toggle" in data[0].text.lower()
...
def test_caps_channel_on(hallo_getter):
    ...
    test_chan1.use_caps_lock = False
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, test_chan, test_user, "channel caps other_channel on")
    )
    data = test_server.get_send_data(1, test_chan, EventMessage)
    assert "error" not in data[0].text.lower()
    assert "caps lock set on" in data[0].text.lower()
...
def test_caps_on_channel(hallo_getter):
    ...
    test_chan1.use_caps_lock = False
    hallo.function_dispatcher.dispatch(
        EventMessage(test_server, test_chan, test_user, "channel caps on other_channel")
    )
    data = test_server.get_send_data(1, test_chan, EventMessage)
    assert "error" not in data[0].text.lower()
    assert "caps lock set on" in data[0].text.lower()
