...
#        del tempcalc, bracket, runncalc, nextchat, tempans
    ...
    def fn_boop(self,args,client,destination):
        ...
        if(len(args)>=2):
            if(args[0][0]=='#'):
                online = ''.join(ircbot_chk.chk_recipientonline(destination[0],[args[1]]))
                if(online==' ' or online==''):
                ...
                online = ''.join(ircbot_chk.chk_recipientonline(destination[0],[args[0]]))
                if(online==' ' or online==''):
                    return 'No one called "' + args + '" is online.'
                else:
            ...
            online = ''.join(ircbot_chk.chk_recipientonline(destination[0],args))
            if(online==' ' or online==''):
    ...
    def fn_cupcake(self, args, client, destination):
        ...
        online = ircbot_chk.chk_recipientonline(destination[0],args.split()[0])
        if(online==' ' or online==''):
    ...
    def fn_speak(self,args,client,destination):
       ...
       #     return client + ": " + self.megahal.get_reply_nolearn(args)
            return "*woof!!!*"
        elif(ircbot_chk.chk_ops(destination[0],client)):
            return "*woof!*"
        else:
            return "*woof*"
    ...
    def fn_silencetherabble(self,args,client,destination):
        ...
        if(ircbot_chk.chk_god(destination[0],client) and destination[1].lower() == '#ecco-the-dolphin'):
            names = ircbot_chk.chk_names(destination[0],destination[1])
    ...
    def fn_amiregistered(self,args,client,destination):
        ...
        if(ircbot_chk.chk_userregistered(destination[0],client)):
