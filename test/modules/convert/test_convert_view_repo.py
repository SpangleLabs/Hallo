...
class ConvertViewRepoTest(ConvertFunctionTestBase, unittest.TestCase):
    ...
    def test_type_specified_unit_incorrect(self):
        self.function_dispatcher.dispatch(EventMessage(
            self.server, None, self.test_user, "convert view repo type=test_type1 unit=no_unit"
        ))
        data = self.server.get_send_data(1, self.test_user, EventMessage)
    ...
    def test_type_specified_unit_different_type(self):
        self.function_dispatcher.dispatch(EventMessage(
            self.server, None, self.test_user, "convert view repo type=test_type1 unit=unit2a"
        ))
        data = self.server.get_send_data(1, self.test_user, EventMessage)
        assert "unrecognised unit specified" in data[0].text.lower()
        assert self.mock_view_type.arg is None
    ...
    def test_type_and_unit_specified(self):
        self.function_dispatcher.dispatch(EventMessage(
            self.server, None, self.test_user, "convert view repo type=test_type1 unit=unit1b"
        ))
        data = self.server.get_send_data(1, self.test_user, EventMessage)
    ...
    def test_group_specified_prefix_incorrect(self):
        self.function_dispatcher.dispatch(EventMessage(
            self.server, None, self.test_user, "convert view repo group=test_group1 prefix=no_prefix"
        ))
        data = self.server.get_send_data(1, self.test_user, EventMessage)
    ...
    def test_group_specified_prefix_different_group(self):
        self.function_dispatcher.dispatch(EventMessage(
            self.server, None, self.test_user, "convert view repo group=test_group1 prefix=prefix2a"
        ))
        data = self.server.get_send_data(1, self.test_user, EventMessage)
        assert "unrecognised prefix specified" in data[0].text.lower()
        assert self.mock_view_group.arg is None
    ...
    def test_group_and_prefix_specified(self):
        self.function_dispatcher.dispatch(EventMessage(
            self.server, None, self.test_user, "convert view repo group=test_group1 prefix=prefix1a"
        ))
        data = self.server.get_send_data(1, self.test_user, EventMessage)
    ...
    def test_group_and_prefix_abbr_specified(self):
        self.function_dispatcher.dispatch(EventMessage(
            self.server, None, self.test_user, "convert view repo group=test_group1 prefix=pref1a"
        ))
        data = self.server.get_send_data(1, self.test_user, EventMessage)
        assert self.output_prefix in data[0].text.lower()
        assert self.mock_view_group.arg is None
    ...
    def test_nothing_specified(self):
        self.function_dispatcher.dispatch(EventMessage(
            self.server, None, self.test_user, "convert view repo"
        ))
        data = self.server.get_send_data(1, self.test_user, EventMessage)
