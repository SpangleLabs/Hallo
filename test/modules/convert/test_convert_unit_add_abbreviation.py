...
class ConvertUnitAddAbbreviationTest(ConvertFunctionTestBase, unittest.TestCase):
    ...
    def test_specified_unit_ambiguous_but_type_specified(self):
        ...
        assert (
            len(self.test_unit1b.abbr_list) == abbr_unit1b + 1
        ), "Should have added abbreviation to type1 same_name"
    ...
    def test_specified_abbreviation(self):
        ...
        assert (
            len(self.test_unit1a.abbr_list) == abbr_unit1a + 1
        ), "Should have added abbreviation to unit1a"
        assert self.test_unit1a.abbr_list[0] == "u1a"
        assert (
            len(self.test_unit1b.abbr_list) == abbr_unit1b
        ), "Shouldn't have added abbreviation to unit1b"
        assert (
            len(self.test_unit2a.abbr_list) == abbr_unit2a
        ), "Shouldn't have added abbreviation to unit2a"
        assert (
            len(self.test_unit2b.abbr_list) == abbr_unit2b
    ...
    def test_multi_word_ambiguous(self):
        ...
        assert (
            len(self.test_unit1a.abbr_list) == abbr_unit1a
        ), "Shouldn't have added abbreviation to unit1a"
        assert (
            len(self.test_unit1b.abbr_list) == abbr_unit1b
        ), "Shouldn't have added abbreviation to unit1b"
    ...
    def test_no_abbreviation(self):
        ...
        data = self.server.get_send_data(1, self.test_user, EventMessage)
        assert (
            "must specify both a unit name and an abbreviation" in data[0].text.lower()
        )
        assert (
            len(self.test_unit1a.abbr_list) == abbr_unit1a
