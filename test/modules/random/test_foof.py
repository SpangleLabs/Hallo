...
class FoofTest(TestBase, unittest.TestCase):
    ...
    def test_short_doof(self):
        for x in range(21):
            ...
            self.function_dispatcher.dispatch(
                EventMessage(self.server, None, self.test_user, "fooooooof")
            )
    ...
    def test_medium_doof(self):
        for x in range(21, 41):
            ...
            # Check
            self.function_dispatcher.dispatch(
                EventMessage(self.server, None, self.test_user, "foof")
            )
            data = self.server.get_send_data(1, self.test_user, EventMessage)
    ...
    def test_long_doof(self):
        for x in range(41, 60):
            ...
            # Check
            self.function_dispatcher.dispatch(
                EventMessage(self.server, None, self.test_user, "foooof")
            )
            data = self.server.get_send_data(1, self.test_user, EventMessage)
            assert (
    ...
    def test_passive_foof(self):
        ...
        self.function_dispatcher.dispatch_passive(
            EventMessage(self.server, self.test_chan, self.test_user, "foof")
        )
