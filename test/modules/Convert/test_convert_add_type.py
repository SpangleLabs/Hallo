...
class ConvertAddTypeTest(ConvertFunctionTestBase, unittest.TestCase):
    ...
    def test_duplicate_type_name(self):
        ...
        self.function_dispatcher.dispatch(EventMessage(
            self.server, None, self.test_user, "convert add type test_type1 unit=unit2"
        ))
    ...
    def test_no_base_unit(self):
        num_types = len(self.test_repo.type_list)
        self.function_dispatcher.dispatch(EventMessage(
            self.server, None, self.test_user, "convert add type new_type"
        ))
        data = self.server.get_send_data(1, self.test_user, EventMessage)
    ...
    def test_create_new_type(self):
        num_types = len(self.test_repo.type_list)
        self.function_dispatcher.dispatch(EventMessage(
            self.server, None, self.test_user, "convert add type new_type unit=unit2"
        ))
        data = self.server.get_send_data(1, self.test_user, EventMessage)
    ...
    def test_create_new_type_with_decimals(self):
        num_types = len(self.test_repo.type_list)
        self.function_dispatcher.dispatch(EventMessage(
            self.server, None, self.test_user, "convert add type new_type unit=unit2 decimals=5"
        ))
        data = self.server.get_send_data(1, self.test_user, EventMessage)
        assert "created new type \"new_type\"" in data[0].text.lower(), "Should specify new type name"
