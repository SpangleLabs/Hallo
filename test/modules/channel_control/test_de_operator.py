...
def test_deop_1priv_channel_not_known(hallo_getter):
    ...
    chan1_hallo.is_op = True
    try:
        hallo.function_dispatcher.dispatch(EventMessage(serv1, None, user1, "deop other_channel"))
        data = serv1.get_send_data(1, user1, EventMessage)
        assert "error" in data[0].text.lower()
...
def test_deop_1priv_not_in_channel(hallo_getter):
    ...
    chan1_hallo.is_op = True
    try:
        hallo.function_dispatcher.dispatch(EventMessage(serv1, None, user1, "deop test_chan2"))
        data = serv1.get_send_data(1, user1, EventMessage)
        assert "error" in data[0].text.lower()
...
def test_deop_1priv_user_not_there(hallo_getter):
    ...
    chan1_hallo.is_op = True
    try:
        hallo.function_dispatcher.dispatch(EventMessage(serv1, None, user1, "deop test_chan1"))
        data = serv1.get_send_data(1, user1, EventMessage)
        assert "error" in data[0].text.lower()
...
def test_deop_1_chan_user_not_there(hallo_getter):
    ...
    try:
        hallo.function_dispatcher.dispatch(EventMessage(serv1, chan1, user1, "deop test_chan2"))
        data = serv1.get_send_data(1, chan1, EventMessage)
        assert "error" in data[0].text.lower()
...
def test_deop_1_user_not_here(hallo_getter):
    ...
    chan1_hallo.is_op = True
    try:
        hallo.function_dispatcher.dispatch(EventMessage(serv1, chan1, user1, "deop test_user2"))
        data = serv1.get_send_data(1, chan1, EventMessage)
        assert "error" in data[0].text.lower()
...
def test_deop_2_chan_user_not_known(hallo_getter):
    ...
    chan2_hallo.is_op = True
    try:
        hallo.function_dispatcher.dispatch(EventMessage(serv1, chan1, user1, "deop test_chan2 test_user3"))
        data = serv1.get_send_data(1, chan1, EventMessage)
        assert "error" in data[0].text.lower()
...
def test_deop_2_chan_no_power(hallo_getter):
    ...
    chan2_hallo.is_op = False
    try:
        hallo.function_dispatcher.dispatch(EventMessage(serv1, chan1, user1, "deop test_chan2 test_user2"))
        data = serv1.get_send_data(1, chan1, EventMessage)
        assert "error" in data[0].text.lower()
        assert "don't have power" in data[0].text.lower()
    finally:
...
def test_deop_2_user_not_in_channel(hallo_getter):
    ...
    chan2_user1.is_op = False
    try:
        hallo.function_dispatcher.dispatch(EventMessage(serv1, chan1, user1, "deop test_user2 test_chan2"))
        data = serv1.get_send_data(1, chan1, EventMessage)
        assert "error" in data[0].text.lower()
...
def test_deop_2_user_user_not_known(hallo_getter):
    ...
    chan2_hallo.is_op = True
    try:
        hallo.function_dispatcher.dispatch(EventMessage(serv1, chan1, user1, "deop test_user3 test_chan2"))
        data = serv1.get_send_data(1, chan1, EventMessage)
        assert "error" in data[0].text.lower()
        assert "test_user3 is not known" in data[0].text.lower()
    finally:
