...
def test_pass_off(hallo_getter):
    ...
    hallo.function_dispatcher.dispatch(EventMessage(test_server, test_chan, test_user, "channel password"))
...
def test_pass_null(hallo_getter):
    ...
    hallo.function_dispatcher.dispatch(EventMessage(
        test_server, test_chan, test_user, "channel password none"
    ))
    data = test_server.get_send_data(1, test_chan, EventMessage)
    assert "error" not in data[0].text.lower()
    assert "password disabled" in data[0].text.lower()
...
def test_pass_set(hallo_getter):
    ...
    hallo.function_dispatcher.dispatch(EventMessage(
        test_server, test_chan, test_user, "channel password test_pass"
    ))
    data = test_server.get_send_data(1, test_chan, EventMessage)
    assert "error" not in data[0].text.lower()
...
def test_pass_chan_set(hallo_getter):
    ...
    hallo.function_dispatcher.dispatch(EventMessage(
        test_server, test_chan, test_user, "channel password other_channel test_pass"
    ))
    data = test_server.get_send_data(1, test_chan, EventMessage)
    assert "error" not in data[0].text.lower()
    assert "password set" in data[0].text.lower()
