...
def test_no_args(hallo_getter):
    ...
    hallo.function_dispatcher.dispatch(EventMessage(test_server, test_channel, test_user, "leave"))
...
def test_channel_name(hallo_getter):
    ...
    hallo.function_dispatcher.dispatch(EventMessage(
        test_server, test_channel, test_user, "leave " + test_channel.name
    ))
    data = test_server.get_send_data(1, test_channel, EventMessage)
    chans = test_server.get_left_channels(1)
    assert chans[0] == test_channel
...
def test_no_args_privmsg(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"server_control"})
    hallo.function_dispatcher.dispatch(EventMessage(test_server, None, test_user, "leave"))
...
def test_other_channel_name(hallo_getter):
    ...
    hallo.function_dispatcher.dispatch(EventMessage(
        test_server, test_channel, test_user, "leave " + other.name
    ))
    data = test_server.get_send_data(1, test_channel, EventMessage)
    chans = test_server.get_left_channels(1)
...
def test_channel_name_privmsg(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"server_control"})
    hallo.function_dispatcher.dispatch(EventMessage(
        test_server, None, test_user, "leave " + test_channel.name
    ))
    data = test_server.get_send_data(1, test_user, EventMessage)
    chans = test_server.get_left_channels(1)
    assert chans[0] == test_channel
...
def test_not_in_channel(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"server_control"})
    hallo.function_dispatcher.dispatch(EventMessage(
        test_server, test_channel, test_user, "leave #not_in_channel"
    ))
    data = test_server.get_send_data(1, test_channel, EventMessage)
    test_server.get_left_channels(0)
    assert "error" in data[0].text.lower()
...
def test_server_specified_first(hallo_getter):
    ...
    # Send command
    hallo.function_dispatcher.dispatch(EventMessage(
        test_server, test_channel, test_user, "leave server=" + test_serv.name + " " + test_chan.name
    ))
    # Check response data
    data = test_server.get_send_data(1, test_channel, EventMessage)
    test_server.get_left_channels(0)
...
def test_server_specified_second(hallo_getter):
    ...
    # Send command
    hallo.function_dispatcher.dispatch(EventMessage(
        test_server, test_channel, test_user, "leave " + test_chan.name + " server=" + test_serv.name
    ))
    # Check response data
    data = test_server.get_send_data(1, test_channel, EventMessage)
    test_server.get_left_channels(0)
...
def test_server_not_on_server(hallo_getter):
    ...
    # Send command
    hallo.function_dispatcher.dispatch(EventMessage(
        test_server, test_channel, test_user, "leave server=not_a_server " + test_channel.name
    ))
    # Check response data
    data = test_server.get_send_data(1, test_channel, EventMessage)
    test_server.get_left_channels(0)
