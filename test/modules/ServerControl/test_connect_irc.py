...
class ConnectIRCTest(TestBase, unittest.TestCase):
    ...
    def test_connect_specify_irc(self):
        ...
        self.function_dispatcher.dispatch(EventMessage(
            self.server, self.test_chan, self.test_user, "connect irc www.example.com:80"
        ))
    ...
    def test_port_in_url(self):
        ...
        # Run command
        self.function_dispatcher.dispatch(EventMessage(
            self.server, self.test_chan, self.test_user, "connect irc www.example.com:" + str(test_port)
        ))
    ...
    def test_port_by_argument(self):
        ...
        # Run command
        self.function_dispatcher.dispatch(EventMessage(
            self.server, self.test_chan, self.test_user, "connect irc www.example.com server_port=" + str(test_port)
        ))
        # Ensure correct response
        data = self.server.get_send_data(1, self.test_chan, EventMessage)
        assert "connected to new irc server" in data[0].text.lower(), "Incorrect output: " + str(data[0].text)
    ...
    def test_address_in_argument(self):
        ...
        # Run command
        self.function_dispatcher.dispatch(EventMessage(
            self.server, self.test_chan, self.test_user, "connect irc " + test_url + " server_port=80"
        ))
        # Ensure correct response
        data = self.server.get_send_data(1, self.test_chan, EventMessage)
        assert "connected to new irc server" in data[0].text.lower(), "Incorrect output: " + str(data[0].text)
    ...
    def test_address_by_argument(self):
        ...
        # Run command
        self.function_dispatcher.dispatch(EventMessage(
            self.server, self.test_chan, self.test_user, "connect irc server_address=" + test_url + " server_port=80"
        ))
        # Ensure correct response
        data = self.server.get_send_data(1, self.test_chan, EventMessage)
        assert "connected to new irc server" in data[0].text.lower(), "Incorrect output: " + str(data[0].text)
    ...
    def test_non_int_port_failure(self):
        # Run command
        self.function_dispatcher.dispatch(EventMessage(
            self.server, self.test_chan, self.test_user, "connect irc example.com server_port=abc"
        ))
        # Check response
        data = self.server.get_send_data(1, self.test_chan, EventMessage)
        assert "error" in data[0].text.lower(), "Connect didn't respond with an error."
        assert "invalid port" in data[0].text.lower(), "Connect returned the wrong error (" + str(data[0].text) + ")"
    ...
    def test_null_address(self):
        ...
        data = self.server.get_send_data(1, self.test_chan, EventMessage)
        assert "error" in data[0].text.lower(), "Connect didn't respond with an error."
        assert "no server address" in data[0].text.lower(), "Connect returned the wrong error (" + str(
            data[0].text) + ")"
    ...
    def test_specified_server_name(self):
        ...
        self.function_dispatcher.dispatch(EventMessage(
            ...
            "connect irc " + test_server + " server_port=80 server_name=" + test_name
        ))
        # Ensure correct response
        data = self.server.get_send_data(1, self.test_chan, EventMessage)
        assert "connected to new irc server" in data[0].text.lower(), "Incorrect output: " + str(data[0].text)
    ...
    def test_get_server_name_from_domain(self):
        ...
        # Run command
        self.function_dispatcher.dispatch(EventMessage(
            self.server, self.test_chan, self.test_user, "connect irc " + test_server + " server_port=80"
        ))
        # Ensure correct response
        data = self.server.get_send_data(1, self.test_chan, EventMessage)
        assert "connected to new irc server" in data[0].text.lower(), "Incorrect output: " + str(data[0].text)
    ...
    def test_auto_connect_true(self):
        # Run command
        self.function_dispatcher.dispatch(EventMessage(
            self.server, self.test_chan, self.test_user, "connect irc www.example.com:80 auto_connect=true"
        ))
        # Ensure correct response
        data = self.server.get_send_data(1, self.test_chan, EventMessage)
        assert "connected to new irc server" in data[0].text.lower(), "Incorrect output: " + str(data[0].text)
    ...
    def test_auto_connect_false(self):
        # Run command
        self.function_dispatcher.dispatch(EventMessage(
            self.server, self.test_chan, self.test_user, "connect irc www.example.com:80 auto_connect=false"
        ))
        # Ensure correct response
        data = self.server.get_send_data(1, self.test_chan, EventMessage)
        assert "connected to new irc server" in data[0].text.lower(), "Incorrect output: " + str(data[0].text)
    ...
    def test_server_nick_specified(self):
        ...
        # Run command
        self.function_dispatcher.dispatch(EventMessage(
            self.server, self.test_chan, self.test_user, "connect irc www.example.com:80 nick=" + test_nick
        ))
        # Ensure correct response
        data = self.server.get_send_data(1, self.test_chan, EventMessage)
        assert "connected to new irc server" in data[0].text.lower(), "Incorrect output: " + str(data[0].text)
    ...
    def test_server_prefix_specified_string(self):
        ...
        # Run command
        self.function_dispatcher.dispatch(EventMessage(
            self.server, self.test_chan, self.test_user, "connect irc www.example.com:80 prefix=" + test_prefix
        ))
        # Ensure correct response
        data = self.server.get_send_data(1, self.test_chan, EventMessage)
        assert "connected to new irc server" in data[0].text.lower(), "Incorrect output: " + str(data[0].text)
    ...
    def test_server_prefix_specified_none(self):
        # Run command
        self.function_dispatcher.dispatch(EventMessage(
            self.server, self.test_chan, self.test_user, "connect irc www.example.com:80 prefix=none"
        ))
        # Ensure correct response
        data = self.server.get_send_data(1, self.test_chan, EventMessage)
        assert "connected to new irc server" in data[0].text.lower(), "Incorrect output: " + str(data[0].text)
    ...
    def test_full_name_specified_string(self):
        ...
        # Run command
        self.function_dispatcher.dispatch(EventMessage(
            self.server, self.test_chan, self.test_user, "connect irc www.example.com:80 full_name=" + test_name
        ))
        # Ensure correct response
        data = self.server.get_send_data(1, self.test_chan, EventMessage)
        assert "connected to new irc server" in data[0].text.lower(), "Incorrect output: " + str(data[0].text)
    ...
    def test_nickserv_nick_inherit(self):
        ...
        # Run command
        self.function_dispatcher.dispatch(EventMessage(
            test_serv_irc, test_chan_irc, test_user_irc, "connect irc example.com:80"
        ))
        # Can't check response because I'm using a ServerIRC instead of a ServerMock
        # Find the right server
        assert len(self.hallo.server_list) == 2, "Incorrect number of servers in hallo instance."
    ...
    def test_nickserv_identity_command_specify(self):
        ...
        self.function_dispatcher.dispatch(EventMessage(
            self.server, self.test_chan, self.test_user,
            "connect irc www.example.com:80 nickserv_identity_command=" + test_nickserv_command
        ))
        # Ensure correct response
        data = self.server.get_send_data(1, self.test_chan, EventMessage)
        assert "connected to new irc server" in data[0].text.lower(), "Incorrect output: " + str(data[0].text)
    ...
    def test_nickserv_identity_response_specify(self):
        ...
        self.function_dispatcher.dispatch(EventMessage(
            self.server, self.test_chan, self.test_user,
            "connect irc www.example.com:80 nickserv_identity_resp=" + test_nickserv_response
        ))
        # Ensure correct response
        data = self.server.get_send_data(1, self.test_chan, EventMessage)
        assert "connected to new irc server" in data[0].text.lower(), "Incorrect output: " + str(data[0].text)
    ...
    def test_nickserv_password_specify(self):
        ...
        self.function_dispatcher.dispatch(EventMessage(
            self.server, self.test_chan, self.test_user,
            "connect irc www.example.com:80 nickserv_password=" + test_nickserv_pass
        ))
        # Ensure correct response
        data = self.server.get_send_data(1, self.test_chan, EventMessage)
        assert "connected to new irc server" in data[0].text.lower(), "Incorrect output: " + str(data[0].text)
    ...
    def test_inherit_user_groups_specify_nick(self):
        ...
        # Run command
        self.function_dispatcher.dispatch(EventMessage(
            self.server, self.test_chan, self.test_user, "connect irc www.example.com:80 god=" + test_user
        ))
        # Ensure correct response
        data = self.server.get_send_data(1, self.test_chan, EventMessage)
        assert "connected to new irc server" in data[0].text.lower(), "Incorrect output: " + str(data[0].text)
