...
def test_avg_simple(hallo_getter):
    ...
    hallo.function_dispatcher.dispatch(EventMessage(test_server, None, test_user, "average 2 4"))
...
def test_avg_same(hallo_getter):
    hallo, test_server, test_chan, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(EventMessage(test_server, None, test_user, "average 2 2 2 2 2"))
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_avg_floats(hallo_getter):
    hallo, test_server, test_chan, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(EventMessage(test_server, None, test_user, "average 2.4 3.2 6.6 1.2"))
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_avg_negative(hallo_getter):
    hallo, test_server, test_chan, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(EventMessage(
        test_server, None, test_user, "average -5 5 -10 10 -14 -16 13 17"
    ))
    data = test_server.get_send_data(1, test_user, EventMessage)
