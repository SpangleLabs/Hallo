...
class TrumpTest(TestBase, unittest.TestCase):
    ...
    def test_trump_simple(self):
        ...
        assert "error" not in data[0][0].lower(), "Trump function is returning error."
        assert "IMPERATOR TRUMP!" in data[0][0], "Trump function does not declare imperator trump."
    ...
    def test_trump_str(self):
        ...
        data = self.server.get_send_data(1, self.test_user, Server.MSG_MSG)
        assert "error" not in data[0][0].lower(), "Trump function is not working when given invalid number."
