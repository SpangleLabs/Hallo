...
def test_slowclap(hallo_getter):
    ...
    hallo.function_dispatcher.dispatch(EventMessage(test_server, test_channel, test_user, "slowclap"))
    time_end = time.time()
...
def test_slowclap_privmsg(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"silly"})
    time_start = time.time()
    hallo.function_dispatcher.dispatch(EventMessage(test_server, None, test_user, "slowclap"))
    time_end = time.time()
...
def test_slowclap_chan(hallo_getter):
    ...
    time_start = time.time()
    hallo.function_dispatcher.dispatch(EventMessage(test_server, None, test_user, "slowclap " + test_chan2.name))
    time_end = time.time()
