...
def test_run(hallo_getter):
    ...
    hallo.function_dispatcher.dispatch(EventMessage(test_server, None, test_user, "reply"))
...
def test_reply_passive(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"silly"})
    hallo.function_dispatcher.dispatch_passive(EventMessage(test_server, test_channel, test_user, "beep"))
...
def test_reply_beep(hallo_getter):
    ...
    response = reply_obj.passive_run(EventMessage(test_server, test_channel, test_user, "beep"), hallo)
    assert response.text == "boop"
    # Check that it doesn't respond if beep is in the message
    response = reply_obj.passive_run(
        EventMessage(test_server, test_channel, test_user, "it goes beep"),
        hallo
    )
...
def test_reply_pew(hallo_getter):
    ...
    # Check pewpew
    response = reply_obj.passive_run(
        EventMessage(test_server, test_channel, test_user, "pew"),
        hallo
    )
...
def test_reply_haskell(hallo_getter):
    ...
    user1 = serv1.get_user_by_address("test_user".lower(), "test_user")
    response = reply_obj.passive_run(
        EventMessage(serv1, chan1, user1, "haskell.jpg"),
        hallo
    )
...
def test_reply_podbay_doors(hallo_getter):
    ...
    # Check pod bay doors
    response = reply_obj.passive_run(
        EventMessage(test_server, test_channel, test_user, "open the pod bay doors hallo."),
        hallo
    )
...
def test_reply_irc_client(hallo_getter):
    ...
    response = reply_obj.passive_run(
        EventMessage(test_server, test_channel, test_user, "Which IRC client should I use?"),
        hallo
    )
...
def test_reply_who_hallo(hallo_getter):
    ...
    response = reply_obj.passive_run(
        EventMessage(test_server, test_channel, test_user, "What is hallo?"),
        hallo
    )
...
def test_reply_mfw(hallo_getter):
    ...
    response = reply_obj.passive_run(
        EventMessage(test_server, test_channel, test_user, "MFW"),
        hallo
    )
