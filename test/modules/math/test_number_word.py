...
def test_number_simple(hallo_getter):
    ...
    hallo.function_dispatcher.dispatch(EventMessage(test_server, None, test_user, "number 5"))
...
def test_number_big(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(EventMessage(test_server, None, test_user, "number 295228"))
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_number_teen(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(EventMessage(test_server, None, test_user, "number 17"))
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_number_negative(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(EventMessage(test_server, None, test_user, "number -502"))
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_number_float(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(EventMessage(test_server, None, test_user, "number 2.3"))
    data = test_server.get_send_data(1, test_user, EventMessage)
    assert "two point three" in data[0].text.lower(), "Number word failing for non-integers."
    hallo.function_dispatcher.dispatch(EventMessage(test_server, None, test_user, "number 2.357"))
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_number_american(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(EventMessage(test_server, None, test_user, "number 1000000000 american"))
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_number_british(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(EventMessage(test_server, None, test_user, "number 1000000000 british"))
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_number_european(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(EventMessage(test_server, None, test_user, "number 1000000000 european"))
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_number_calculation(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(EventMessage(test_server, None, test_user, "number 17*5"))
    data = test_server.get_send_data(1, test_user, EventMessage)
...
def test_number_fail(hallo_getter):
    hallo, test_server, test_channel, test_user = hallo_getter({"math"})
    hallo.function_dispatcher.dispatch(EventMessage(test_server, None, test_user, "number seventeen"))
    data = test_server.get_send_data(1, test_user, EventMessage)
