...
class ChooseTest(TestBase, unittest.TestCase):
    ...
    def test_eightball(self):
        ...
        self.function_dispatcher.dispatch(EventMessage(self.server, None, self.test_user, "eight ball"))
    ...
    def test_all_responses(self):
        ...
        for x in range(len(all_responses)):
            ...
            self.function_dispatcher.dispatch(EventMessage(self.server, None, self.test_user, "magic8-ball"))
            data = self.server.get_send_data(1, self.test_user, EventMessage)
            responses.append(data[0].text.lower()[:-1])
            assert data[0].text.lower() in ["{}.".format(x.lower()) for x in all_responses], "Response isn't valid."
        # Check all responses given
        assert len(responses) == len(all_responses), "Not the same number of responses as possible responses"
        assert set(responses) == set([x.lower() for x in all_responses]), "Not all responses are given"
