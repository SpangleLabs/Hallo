...
class RandomColourTest(TestBase, unittest.TestCase):
    ...
    def test_black(self):
        ...
        # Check
        self.function_dispatcher.dispatch(EventMessage(self.server, None, self.test_user, "random colour"))
        data = self.server.get_send_data(1, self.test_user, EventMessage)
        assert "randomly chosen colour is" in data[0].text.lower(), "Should state that a colour is chosen."
        assert "https://www.thecolorapi.com" in data[0].text, "URL should be in response."
    ...
    def test_white(self):
        ...
        assert "rgb(255,255,255)" in data[0].text, "RGB code should be stated as 255,255,255"
        assert self.roller.last_min == 0, "0 should be minimum colour value."
        assert self.roller.last_max == 255, "255 should be max colour value."
        assert self.roller.last_count == 3, "Should ask for 3 numbers."
    ...
    def test_grey(self):
        ...
        assert "rgb(127,127,127)" in data[0].text, "RGB code should be stated as 127,127,127"
        assert self.roller.last_min == 0, "0 should be minimum colour value."
        assert self.roller.last_max == 255, "255 should be max colour value."
        assert self.roller.last_count == 3, "Should ask for 3 numbers."
