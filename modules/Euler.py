...
class Euler(Function):
    ...
    def runFunction(self, numberString):
        ...
        try:
            outputString = "Euler project problem " + numberString + "? I think the answer is: " + str(
                functionObject()) + "."
    ...
    def checkConcatPrimes(self, numberOne, numberTwo):
        checkOne = self.checkPrime(int(str(numberOne) + str(numberTwo)))
        if not checkOne:
            return False
        checkTwo = self.checkPrime(int(str(numberTwo) + str(numberOne)))
        if not checkTwo:
            return False
        return True
    ...
    def euler11(self):
        ...
        for x in range(0, 20):
            for y in range(0, 17):
                product = int(arrBox[x + 20 * y]) * int(arrBox[x + 20 * y + 20]) * int(arrBox[x + 20 * y + 40]) * int(
                    arrBox[x + 20 * y + 60])
        ...
        for x in range(0, 17):
            for y in range(0, 20):
                product = int(arrBox[x + 20 * y]) * int(arrBox[x + 20 * y + 1]) * int(arrBox[x + 20 * y + 2]) * int(
                    arrBox[x + 20 * y + 3])
                if product > biggestProduct:
                    biggestProduct = product
                    answerX = x
        ...
        for x in range(0, 17):
            for y in range(0, 17):
                product = int(arrBox[x + 20 * y]) * int(arrBox[x + 20 * y + 21]) * int(arrBox[x + 20 * y + 42]) * int(
                    arrBox[x + 20 * y + 63])
                if product > biggestProduct:
                    biggestProduct = product
                    answerX = x
        ...
        for x in range(3, 20):
            for y in range(0, 17):
                product = int(arrBox[x + 20 * y]) * int(arrBox[x + 20 * y + 19]) * int(arrBox[x + 20 * y + 38]) * int(
                    arrBox[x + 20 * y + 57])
                if product > biggestProduct:
                    biggestProduct = product
                    answerX = x
    ...
    def euler17(self):
        ...
        for x in range(1, 1001):
            total += len(functionObject.numberWord(str(x)).replace(' ', '').replace('-', ''))
        return total
    ...
    def euler18(self):
        ...
        for x in range(len(arrTriangle)):
            arrTriangleVal[x] = arrTriangle[x].split()
        for row in range(len(arrTriangleVal) - 2, -1, -1):
            for col in range(len(arrTriangleVal[row])):
                if int(arrTriangleVal[row + 1][col]) > int(arrTriangleVal[row + 1][col + 1]):
                    arrTriangleVal[row][col] = int(arrTriangleVal[row][col]) + int(arrTriangleVal[row + 1][col])
                else:
                    arrTriangleVal[row][col] = int(arrTriangleVal[row][col]) + int(arrTriangleVal[row + 1][col + 1])
        return arrTriangleVal[0][0]
                        ...
                        for e in range(5):
                            ...
                            for f in range(4):
                                if str(edigits[f]) not in list(str(productOne)) or productOne > 9999:
                                    failOne = True
                            for g in range(4):
                                if (str(edigits[g]) not in list(
                                        str(productTwo)) or productTwo < 1000 or productTwo > 9999):
                                    failTwo = True
                            if not failOne:
                                print(str(adigit) + str(bdigit) + "*" + str(cdigit) + str(ddigit) + str(
                                    edigit) + "=" + str(productOne))
                                products.append(productOne)
                            if not failTwo:
                                print(str(adigit) + "*" + str(bdigit) + str(cdigit) + str(ddigit) + str(
                                    edigit) + "=" + str(productTwo))
                ...
                if str(numerator)[0] in str(denominator):
                    ...
                    if denominatorNew != 0:
                        if (numerator / denominator - numeratorNew / denominatorNew) ** 2 < epsilon:
                            ...
                            totalNumeratorFactors = totalNumeratorFactors + numeratorFactorsNew
                            totalDenominatorFactors = totalDenominatorFactors + denominatorFactorsNew
        totalDenominatorFactorsNew = self.listMinus(totalDenominatorFactors,
                                                    self.listIntersection(totalDenominatorFactors,
                                                                          totalNumeratorFactors))
    ...
    def euler60(self):
        ...
        while True:
            ...
            for numOne in bigDict:
                if numOne == num:
                    continue
                check = self.checkConcatPrimes(num, numOne)
                if not check:
                    continue
                bigDict[numOne][num] = {}
                for numTwo in bigDict[numOne]:
                    if numTwo == num:
                        continue
                    check = self.checkConcatPrimes(num, numTwo)
                    if not check:
                        continue
                    ...
                    for numThree in bigDict[numOne][numTwo]:
                        if numThree == num:
                            continue
                        check = self.checkConcatPrimes(num, numThree)
                        if not check:
                            continue
                        ...
                        for numFour in bigDict[numOne][numTwo][numThree]:
                            if numFour == num:
                                continue
                            check = self.checkConcatPrimes(num, numFour)
                            if not check:
                                continue
        ...
        for row in range(len(arr_triangle_val) - 2, -1, -1):
            for col in range(len(arr_triangle_val[row])):
                if int(arr_triangle_val[row + 1][col]) > int(arr_triangle_val[row + 1][col + 1]):
                    arr_triangle_val[row][col] = int(arr_triangle_val[row][col]) + int(arr_triangle_val[row + 1][col])
                else:
                    arr_triangle_val[row][col] = int(arr_triangle_val[row][col]) + int(arr_triangle_val[row + 1][col + 1])
