...
class NightValeWeather(Function):
    ...
    def getYoutubePlaylist(self, playlistId, pageToken=None):
        ...
        apiUrl = "https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=50&playlistId=" + \
                 playlistId + "&fields=" + urllib.parse.quote(apiFields) + "&key=" + apiKey
    ...
    def run(self, line, userObject, destinationObject=None):
        ...
        if colourMatch is None or colourMatch.group(1) is None:
            output = "Randomly chosen colour is: #" + hexCode + " or rgb(" + str(rgbList[0]) + "," + str(
                rgbList[1]) + "," + str(rgbList[2]) + ") " + url
        else:
            colourName = colourMatch.group(1)
            output = "Randomly chosen colour is: " + colourName + " #" + hexCode + " or rgb(" + str(
                rgbList[0]) + "," + str(rgbList[1]) + "," + str(rgbList[2]) + ") " + url
        return output
