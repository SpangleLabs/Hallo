...
class HighScores(Function):
    ...
    def addHighScore(self, gameName, score, userName, data=None):
...
class HigherOrLowerGame(Game):
    ...
    def guessHigher(self):
        ...
        if nextCard.toInt() > lastCard.toInt():
            outputString = "Your " + Commons.ordinal(
                self.mTurns) + " card is " + nextCard.toString() + ", which is higher! "
            outputString += "Congrats! Do you think the next card will be higher or lower?"
            return outputString
        if nextCard.toInt() == lastCard.toInt():
            outputString = "Your " + Commons.ordinal(
                self.mTurns) + " card is " + nextCard.toString() + ", which is the same (that's fine.) "
        ...
        if nextCard.toInt() < lastCard.toInt():
            ...
            # Output message
            outputString = "Your " + Commons.ordinal(
                self.mTurns) + " card is " + nextCard.toString() + ". Sorry, that's lower, you lose."
            if isHighScore:
                outputString += " You managed " + str(
                    self.mTurns - 1) + " cards though, that's a new highscore!" + previousScoreText
            else:
    ...
    def guessLower(self):
        ...
        if nextCard.toInt() < lastCard.toInt():
            outputString = "Your " + Commons.ordinal(
                self.mTurns) + " card is " + nextCard.toString() + ", which is lower! "
            outputString += "Congrats! Do you think the next card will be higher or lower?"
            return outputString
        if nextCard.toInt() == lastCard.toInt():
            outputString = "Your " + Commons.ordinal(
        ...
        if nextCard.toInt() > lastCard.toInt():
            ...
            # Output message
            outputString = "Your " + Commons.ordinal(
                self.mTurns) + " card is " + nextCard.toString() + ". Sorry, that's higher, you lose."
            if isHighScore:
                outputString += " You managed " + str(
                    self.mTurns - 1) + " cards though, that's a new highscore!" + previousScoreText
            else:
    ...
    def quitGame(self):
        ...
        # Create output
        if isHighScore:
            return "Sorry to see you quit, you had managed " + str(
                self.mTurns - 1) + " cards, which is a new highscore!" + previousScoreText
        else:
...
class BlackjackGame(Game):
    ...
    def startGame(self):
        ...
        if (self.mPlayerHand.containsValue(Card.CARD_ACE) and any([self.mPlayerHand.containsValue(value) for value in
        ...
        outputString += " The dealer has a " + secondCard.toString() + " and another, covered, card. " \
    ...
    def hit(self):
        ...
        if self.mPlayerHand.sumTotal() > 21:
            self.mLost = True
            return outputString + " which means your hand sums to " + str(
                self.mPlayerHand.sumTotal()) + ". You've gone bust. You lose, sorry."
    ...
    def stick(self):
        ...
        dealerNewCards = 0
        if (self.mDealerHand.blackjackTotal() < 17 or (
                self.mDealerHand.blackjackTotal() == 17 and self.mDealerHand.containsValue(Card.CARD_ACE))):
            dealerNewCards += 1
        ...
        elif playerSum < self.mDealerHand.blackjackTotal() <= 21:
    ...
    def quitGame(self):
        ...
        return "You have quit the game. You had " + str(
            self.mPlayerHand.blackjackTotal()) + " and the dealer had " + str(self.mDealerHand.blackjackTotal()) + "."
...
class DDRGame(Game):
    ...
    def run(self):
        ...
        if self.checkHighScore(winnerPlayer):
            self.updateHighScore(winnerPlayer)
            serverObject.send(winnerPlayer.getName() + " has set a new DDR highscore with " + str(
                self.mPlayerDict[winnerPlayer]['hits']) + " hits and " + str(
                self.mPlayerDict[winnerPlayer]['lag']) + " lag!", self.mChannel, "message")
            # Game ended
