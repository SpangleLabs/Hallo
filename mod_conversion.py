...
#            return "All conversion aliases: " + ', '.join([alias + '->' + convert['alias'][alias] for alias in convert['alias']]) + "."
    ...
    def fnn_convert_process_string(self,convert,args,client,destination):
        ...
        if(len(from_to)==1):
            ...
            if(unit_type is None):
                list_to = mod_conversion.fnn_convert_check_alias(self,convert,from_to[1])
                list_match = mod_conversion.fnn_convert_combine_lists(self,convert,list_from,list_to)
                if(len(list_match)==1):
